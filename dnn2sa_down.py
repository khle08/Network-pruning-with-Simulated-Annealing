...
class NetworkOptimization(object):
    ...
    def __init__(self, batch_size, learning_rate, mask_ratio,
        ...
        if data == 'mnist':
            ...
            train_dataset = Loader('/Volumes/TOSHIBA EXT 1/Database/1D_lab/Chess',
                                   img_size=128, train=0.8, test=0, proc=False)
            test_dataset = Loader('/Volumes/TOSHIBA EXT 1/Database/1D_lab/Chess',
                                  img_size=128, train=0, test=0.2, proc=False)
        elif data == 'scene':
            train_dataset = Loader(
                '/Volumes/TOSHIBA EXT 1/Database/1D_lab/Scene/seg_train/seg_train',
                img_size=128, train=1.0, test=0, proc=False)
            test_dataset = Loader(
                '/Volumes/TOSHIBA EXT 1/Database/1D_lab/Scene/seg_test/seg_test',
                img_size=128, train=1.0, test=0, proc=False)
        # --------------------------------------------------------------
    ...
    def fit(self, epoch_num, save_step, freeze=False):
        ...
        if freeze:
            ...
            self.model.fc3.weight.requires_grad = False
            self.model.fc1.bias.requires_grad = False
            self.model.fc3.bias.requires_grad = False
...
if __name__ == '__main__':
    ...
    parser.add_argument('--reduce', type=str, default='1,2,5,10,20,40,50')
    ...
    REDUCE = np.array(opt.reduce.split(',')).astype(int) * 0.01
    ...
    for m in METRO:
        folder = '{}_edge{}/metropolis_{}'.format(DATA, EDGE, m)
        ...
        for r in REDUCE:
            ...
            for time in tqdm(range(REPEAT), desc=desc,
                ...
                for i in range(100 // int(r * 100) - 1):
                    ...
                    np.savetxt(os.path.join(folder, 'w{}_reduce{}.txt'.format((i + 1) * 2, r * 100)),
                               nnOpt.model.fc2.weight.data.cpu().numpy())
                    np.savetxt(os.path.join(folder, 'm{}_reduce{}.txt'.format((i + 1) * 2, r * 100)),
